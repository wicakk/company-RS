...
<section class="relative overflow-hidden bg-slate-100 dark:bg-slate-950"
  ...
  <div class="relative py-16 min-h-[80vh] flex flex-col justify-center">

    {{-- ── CAROUSEL TRACK ── --}}
    <div class="relative w-full flex items-center justify-center overflow-hidden" style="height: 520px;">

      @foreach($heroSlides as $i => $slide)
      <div
        class="absolute rounded-3xl overflow-hidden cursor-pointer"
      ...
      >
        {{-- Background --}}
        @if($slide->bg_image)
          <img src="{{ $slide->bg_image_url }}" alt="" class="absolute inset-0 w-full h-full object-cover">
        ...
        <div class="absolute top-10 right-10 w-56 h-56 bg-primary-500/10 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute bottom-10 left-10 w-72 h-72 bg-primary-600/10 rounded-full blur-3xl pointer-events-none"></div>

        {{-- Content --}}
        <div class="absolute inset-0 flex items-center z-10 px-10 lg:px-14"
          ...
          <div class="grid lg:grid-cols-2 gap-12 items-center w-full">
            ...
            <div class="text-white">
              ...
              <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20
                           text-white/90 text-xs font-semibold px-4 py-2 rounded-full mb-6">
                <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                {{ $slide->badge_text }}
              </div>
              @endif

              <h1 class="font-bold text-3xl sm:text-4xl lg:text-5xl leading-tight mb-4">
                {!! nl2br(e($slide->title)) !!}
              </h1>

              @if($slide->subtitle)
              <p class="text-white/70 text-base leading-relaxed mb-8 max-w-xl">{{ $slide->subtitle }}</p>
              @endif

              <div class="flex flex-col sm:flex-row gap-4">
                @if($slide->btn1_label && $slide->btn1_url)
                <a href="{{ $slide->btn1_url }}"
                   class="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-white text-primary-700
                          font-bold rounded-2xl hover:bg-primary-50 transition-all hover:shadow-xl text-sm">
                  {{ $slide->btn1_label }}
                </a>
                @endif
                @if($slide->btn2_label && $slide->btn2_url)
                <a href="{{ $slide->btn2_url }}"
                   class="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-white/10 border
                          border-white/30 backdrop-blur-sm text-white font-semibold rounded-2xl
              ...
              @if($slide->show_stats)
              <div class="flex flex-wrap gap-8 mt-10 pt-8 border-t border-white/20">
                @foreach([
                ...
                <div>
                  ...
                  <div class="text-white/60 text-xs mt-1">{{ $s[1] }}</div>
                </div>
                @endforeach
              </div>
              @endif
            ...
            <div class="hidden lg:block">
              <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-7 text-white">
                <div class="flex items-center gap-4 mb-5">
                  <div class="w-11 h-11 rounded-2xl bg-primary-400/30 flex items-center justify-center">
                    <svg class="w-5 h-5 text-primary-200" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/>
                    </svg>
                  </div>
                  <div>
                    <div class="font-bold">Layanan Tersedia</div>
                    <div class="text-white/60 text-xs">Tersedia sekarang</div>
                  </div>
                </div>
    ...
    {{-- Arrows --}}
    <button @click="prev()"
      class="absolute left-4 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white shadow-lg
             border border-slate-200/80 text-slate-500 hover:text-slate-800 hover:shadow-xl
    ...
    <button @click="next()"
      class="absolute right-4 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-white shadow-lg
             border border-slate-200/80 text-slate-500 hover:text-slate-800 hover:shadow-xl
             transition-all duration-300 flex items-center justify-center group">
      <svg class="w-5 h-5 transition-transform duration-300 group-hover:translate-x-0.5"
           fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    {{-- Progress bar --}}
    <div class="mt-3 mx-auto w-48 h-0.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
...
function heroSlider() {
  return {
    ...
    getSlideStyle(index) {
      ...
      const base = `transition: ${transition}; position: absolute;`;
      ...
      if (offset === 0) {
        ...
        // Farther slides — fade out behind the active card
        const goLeft = offset < 0;
        return `${base}
          width: min(22vw, 340px);
          height: 420px;
          ${goLeft ? 'left: 0;' : 'right: 0; left: auto;'}
          transform: ${goLeft ? 'translateX(-40px)' : 'translateX(40px)'} scale(0.85);
          z-index: 5;
          opacity: 0;
    ...
    startAutoplay() {
      ...
      const tick = (ts) => {
        ...
        if (this.progress >= 100) { this.next(); start = ts; }
