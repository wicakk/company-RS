...
{{-- 
  Cara pemakaian di home.blade.php: 
  Ganti section HERO yang lama dengan:
  ...
  <div class="relative min-h-[90vh] flex items-center">
    ...
    <div class="absolute inset-0">
      ...
      <div
        ...
        :class="current === {{ $i }} ? 'opacity-100 scale-100' : 'opacity-0 scale-105'"
        style="z-index: {{ $i === 0 ? 1 : 0 }}"
        :style="current === {{ $i }} ? 'z-index:1' : 'z-index:0'"
      >
        @if($slide->bg_image)
        ...
        {{-- Decorative blobs --}}
        <div class="absolute top-20 right-20 w-72 h-72 bg-primary-500/10 rounded-full blur-3xl animate-pulse-slow"></div>
    ...
    {{-- Content --}}
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 relative" style="z-index:10">
      @foreach($heroSlides as $i => $slide)
      <div
        class="grid lg:grid-cols-2 gap-16 items-center absolute inset-x-4 sm:inset-x-6 lg:inset-x-8 top-1/2 -translate-y-1/2 transition-all duration-700"
        :class="current === {{ $i }} ? 'opacity-100 translate-y-[-50%]' : 'opacity-0 translate-y-[-40%] pointer-events-none'"
      >
        {{-- Left --}}
        <div class="text-white">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            ...
            <a href="{{ $slide->btn1_url }}"
              ...
              {!! $slide->btn1_icon ? '<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="'.$slide->btn1_icon.'"/></svg>' : '' !!}
              {{ $slide->btn1_label }}
            </a>
            @endif
            ...
            <a href="{{ $slide->btn2_url }}"
              ...
              {!! $slide->btn2_icon ? '<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="'.$slide->btn2_icon.'"/></svg>' : '' !!}
              {{ $slide->btn2_label }}
            </a>
            @endif
          </div>
    ...
    <div class="absolute bottom-0 left-0 z-20 h-0.5 bg-white/20 w-full">
      <div class="h-full bg-primary-400 transition-none" :style="'width:' + progress + '%'" style="transition: width 0.1s linear"></div>
    </div>
    @endif
  </div>
